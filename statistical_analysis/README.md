...
Main question to decide is whether you want the abundance at each taxonomic levels, or the number of OTUs (i.e. diversity) at each level


```R
# diversity
obj <- list(data.frame(cbind(taxData[,1,drop=F],1)[,2,drop=F]), taxData,as.data.frame(colData(dds)))

# abundance - with variance stabilizing transformation (probably best)
obj <-list(as.data.frame(counts(dds,normalize=T)),taxData,as.data.frame(colData(dds)))
...
# Combine and filter low abundance groups 
